    ...
    <div v-else>
        <div v-if="info_list.length">
          <div v-for="(item, index) in info_list" :key="index" class="thumbnail-wrapper">
            <nuxt-link :to="`/projectInfo/${item?.id}`">
              <img :src="item.image?.url" :alt="item?.title" class="thumbnail-list__img"/>
              <div class="overlay">
                <div class="over-text">
                  {{ item.title }}
                </div>
...
export default {
  name: 'WorksInfo',
  setup() {
    const info_list = ref<Prj[]>([]);
    const loading = ref(true);
    const explain = 'ユニットやプロジェクトメンバーとしての活動\nActivities as a unit or project member.';
